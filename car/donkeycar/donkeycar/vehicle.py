...
class Vehicle:
    def __init__(self, mem=None):
        ...
        self.stop_signs_detected = queue.Queue(maxsize=5)
    ...
    def update_parts(self):
        ...
        for entry in self.parts:
            ...
            if run:
                ...
                if isinstance(p, StopSignDetector):
                    if outputs[0] <= 0:
                        print("STOP sign detected")

                    if self.stop_signs_detected.full():
                        self.stop_signs_detected.get()
                    self.stop_signs_detected.put(outputs[0] <= 0)

                    if list(self.stop_signs_detected.queue).count(True) >= 3:
                        # TODO: stop the car - save outputs to memory
